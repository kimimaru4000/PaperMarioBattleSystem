...
namespace PaperMarioBattleSystem
{
    public class TidalWaveCommand : ActionCommand
    {
        ...
        public TidalWaveCommand(BattleAction battleAction, double totalDuration = 3500d, int inputLimit = 14) : base(battleAction)
        {
        ...
        protected override void OnDraw()
        {
            ...
            for (int i = 0; i < ButtonsPressed.Count; i++)
            {
                ...
                SpriteRenderer.Instance.DrawText(AssetManager.Instance.Font, button.ToString(), startDrawLoc + new Vector2(xPosDiff * i, 0), Color.Black, .7f);
            }

            //Show the button that should be pressed next, unless the input limit was reached
            if (ButtonsPressed.Count < InputLimit)
            {
                SpriteRenderer.Instance.DrawText(AssetManager.Instance.Font, NextButtonToPress.ToString(), nextPos, Color.White, .7f);
            }

            SpriteRenderer.Instance.DrawText(AssetManager.Instance.Font, Math.Round(StartTime - Time.ActiveMilliseconds, 2).ToString(), startDrawLoc + new Vector2(0, -20), Color.White, .7f);
        }
    }
}
